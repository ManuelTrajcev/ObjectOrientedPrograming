...
public:
    ...
    virtual int getBodovi() {
        ...
        int bodovi = count / brojOcenki * 100;
        return bodovi;
    }
...
public:
    ...
    bool checkKurs(char *k) {
        for (int i = 0; i < brojKursevi; ++i) {
            if (strcmp(kursevi[i].getIme(), k) == 0) {
                return true;
            }
        }
        return false;
...
void pecatiDemonstratoriKurs(char *kurs, Student **studenti, int n) {
    for (int i = 0; i < n; ++i) {
        ...
        if (casted && casted->getBrojKursevi()) {
            if (casted->checkKurs(kurs)) {
                casted->pecati();
                cout << endl;
            }
        }
